  ...
  return <div className="space-y-6">
    ...
    <section className="relative overflow-hidden rounded-[22px] border border-border bg-card p-6 sm:p-8">
      ...
      <div className="relative flex flex-wrap items-end justify-between gap-5">
        <div className="flex items-center gap-4"><FlagChip code={team.code} w={56} h={37} r={7} /><div><p className="text-xs uppercase tracking-[0.14em] font-bold text-texts">{team.confederation} · {team.group_letter ? <Link href={`/groups?group=${team.group_letter}&tab=stand`} className="underline decoration-primary/50 underline-offset-2 hover:text-primary">Group {team.group_letter}</Link> : 'World Cup 2026'}</p><h1 className="text-3xl font-extrabold text-textp mt-1">{team.name}</h1></div></div>
