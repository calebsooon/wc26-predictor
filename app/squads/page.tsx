...
function TeamCard({ team, matches, onSelect }: { team: Team; matches: Match[]; onSelect: () => void }) {
  ...
  return (
      <Card hover className="overflow-hidden h-full border-border group-hover:border-primary/50">
        <button onClick={onSelect} className="relative h-[142px] w-full p-5 text-left text-white overflow-hidden" style={{ background: gradient }}>
          ...
          <div className="relative flex items-start justify-between gap-3">
            ...
            <div className="flex items-center gap-1.5">
              ...
              {team.crest_url && <Image src={team.crest_url} alt="" width={40} height={40} className="h-10 w-10 object-contain drop-shadow-md" />}
            </div>
          </div>
  ...
  return <div className="space-y-6">
    ...
    <section className="relative overflow-hidden rounded-[22px] border border-border bg-card p-6 sm:p-8">
      ...
      <div className="relative flex flex-wrap items-end justify-between gap-5">
        <div className="flex items-center gap-4"><div className="flex items-center gap-3 shrink-0">{team.crest_url && <Image src={team.crest_url} alt="" width={56} height={56} className="h-14 w-14 object-contain drop-shadow-lg" />}<FlagChip code={team.code} w={team.crest_url ? 30 : 56} h={team.crest_url ? 20 : 37} r={team.crest_url ? 4 : 7} /></div><div><p className="text-xs uppercase tracking-[0.14em] font-bold text-texts">{team.confederation} · {team.group_letter ? <Link href={`/groups?group=${team.group_letter}&tab=stand`} className="underline decoration-primary/50 underline-offset-2 hover:text-primary">Group {team.group_letter}</Link> : 'World Cup 2026'}</p><h1 className="text-3xl font-extrabold text-textp mt-1">{team.name}</h1></div></div>
