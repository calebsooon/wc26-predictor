...
type Team = { code: string; name: string; confederation: Confederation; group_letter: string | null; is_host: boolean; flag_url: string | null; crest_url: string | null; stats: StatMap; source_updated_at?: string | null; updated_at: string }
...
// FIFA's distance feed is normally metres, while the official UI labels the
// same value in kilometres. Preserve small already-normalised values too.
const displayMetricValue = (key: string, value: number) => key === 'total_distance' && value >= 1000 ? value / 1000 : value
const metricLabel = (key: string) => key.replace(/_/g, ' ').replace(/\b\w/g, (letter) => letter.toUpperCase()).replace(/\bXg\b/, 'xG')
const metricSuffix = (key: string) => key === 'total_distance' ? ' km' : key.includes('speed') ? ' km/h' : /rate|percentage|accuracy|possession|conversion/.test(key) ? '%' : ''
...
function TeamProfile({ detail, onBack }: { detail: TeamDetail; onBack: () => void }) {
  ...
  const leaders = (key: string) => [...activePlayers].filter((player) => stat(player.stats, key) > 0).sort((a, b) => stat(b.stats, key) - stat(a.stats, key))[0]
  const leadersGK = (key: string) => [...activePlayers].filter((p) => squadGroup(p) === 'Goalkeeper' && stat(p.stats, key) > 0).sort((a, b) => stat(b.stats, key) - stat(a.stats, key))[0]
  ...
  const teamExtraMetrics = Object.entries(team.stats)
    .filter(([key, value]) => !['matches_played', 'goals', 'goals_conceded', 'passes', 'passes_completed', 'attempt_at_goal', 'attempt_at_goal_on_target'].includes(key) && number(value) > 0)
    .sort(([a], [b]) => a.localeCompare(b))
    .map(([key, value]) => [metricLabel(key), displayMetricValue(key, number(value)), metricSuffix(key)] as const)
      ...
      <section>
        ...
        <div id={`player-stats-panel-${playerStatView}`} role="tabpanel" aria-labelledby={`player-stats-tab-${playerStatView}`} className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {playerLeaderViews[playerStatView].map(([label, key, suffix, decimals]) => {
            ...
            return <Leader key={key} label={label} player={p} value={displayMetricValue(key, stat(p?.stats ?? {}, key))} suffix={suffix} decimals={decimals} />
          })}
        </div>
      </section>
...
function TeamPlayerSheet({ player, team, onClose }: { player: TeamPlayer; team: Team; onClose: () => void }) {
  const rows = [
...
export default function SquadsPage() {
  ...
  const latestTeamData = useMemo(() => (directory?.teams ?? []).map((team) => team.source_updated_at ?? team.updated_at).filter(Boolean).sort().at(-1) ?? null, [directory])
  ...
  return <div className="space-y-6 max-w-7xl mx-auto">
    <PageHeader eyebrow="World Cup 2026" title="Teams & squads" sub={`Official FIFA tournament squads, team form, and live tournament leaders.${latestTeamData ? ` Synced ${new Intl.DateTimeFormat(undefined, { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(latestTeamData))}.` : ''}`} />
